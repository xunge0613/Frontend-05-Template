function getStyle(element) {
  ...
  function layout(element) {
    ...
    // 如果父元素没有设置主轴尺寸，例如主轴是width属性，但是父元素没有width，会进入AutoMainSize模式
    ...
    for (let i = 0; i < items.lenght; i++) {
      ...
      if (itemStyle.flex) {
        ...
        if (itemStyle[crossSize] !== null && itemStyle[crossSize] !== (void 0)) {
          crossSpace = Math.max(crossSpace, itemStyle[crossSize]); // 计算行高
        }
        flexLine.push(item);
      } else {
        ...
        if (mainSpace < itemStyle[mainSize]) { // 主轴剩余空间小于子元素的mainSize，需要进行换行
